...
int prte_finalize(void)
{
    ...
    if(timings_my_rank == 0){
        char filename_master[256];
        sprintf(filename_master, "%s/build/p4est_dynres/applications/output/prrte/timings_master.csv", path_base);

        if(NULL != master_timing_list){
            print_list_to_file_master(master_timing_list, filename_master);
            free(master_timing_list);
        }else{
            printf("Master Timing list is NULL\n");
        }
    }
    char filename_daemon[256];
    sprintf(filename_daemon, "%s/build/p4est_dynres/applications/output/prrte/timings_daemon_%d.csv", path_base, PRTE_PROC_MY_NAME->rank);


    if(NULL != daemon_timing_list){
        print_list_to_file_daemon(daemon_timing_list, filename_daemon);
        free(daemon_timing_list);
    }else{
        printf("Daemon Timing list is NULL\n");
    }
