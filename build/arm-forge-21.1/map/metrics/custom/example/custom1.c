...
/*!
 *  The difference in values of the metric (the number of interrupts taken) 
 *  since the callback was last called is copied to out_value. 
 *  If this is the first time the callback is being called, the number of interrupts 
 *  taken since the start of the run is copied instead. This value is modified by the 
 *  Arm HPC tools depending on the value of the divideBySampleTime XML switch 
 *  in \c custom1.xml. If the switch is set to true then out_value is divided by 
 *  the sample interval, else it is left untouched.
 *
