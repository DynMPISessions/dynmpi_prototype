...
#pragma omp parallel shared(newval, values)
            {
#pragma omp for
                for (j = 1; j <= npoints; j++)
                {
...
void output_master(void)  
{
    int i, j, start, npts, buffer[2], istep, source;
...
#pragma omp parallel shared(results, values)
    {
...
int main(int argc, char **argv)
{
    ...
    int threadSupport;
    rcode = MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &threadSupport);
