...
class Position:
    """Print a Position Object

    Position objects should be printed like
    ...
    def to_string(self):
        return '"%s@%s:%d:%d"' % (str(self.val['function_']).strip('"'),
                                  str(self.val['file_']['mFileName']).strip('"'),
                                  self.val['line_'],
                                  self.val['column_'])

class PSVModelItem:
    """Print a stripped down PSV Model Tree.

    Disabled by default, to enable run the following in gdb (or the raw command window)

        (gdb) python ddt.ddt.PSVModelItem.enable()

    This is intended to only show the most relevant information so you can easily
    traverse the tree in DDT's variable view when debugging.
    """
    regex = re.compile('^PSVModelItem$')

    def __init__(self, val):
        self.val = val

    def to_string(self):
        return "{mPosition = %s, mChildren = %s}" % (str(self.val['mPosition']),
                                                     str(self.val['mChildren']))

    @staticmethod
    def enable():
        pretty_printers_dict[PSVModelItem.regex] = lambda val: PSVModelItem(val)

    @staticmethod
    def disable():
        del pretty_printers_dict[PSVModelItem.regex]
...
def build_dictionary ():
    ...
    pretty_printers_dict[re.compile('^Position$')] = lambda val: Position(val)
