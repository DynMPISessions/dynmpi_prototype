...
class AutoStop:
    """Helper class for iterators - only do a fixed number of 'operations' per call to next.

    The number of iterations is 1000 because it was deemed big enough for traversing a RB tree which
    has an upper limit on its depth of 2*log(n+1), so 1000 seemed like a reasonably high limit
    without impacting performance. If using AutoStop for another purpose, this assumption may need
    to be reconsidered.

    Raises `exception` if the limit is exceeded - typically because we're looking at uninitialized
    data and are in a loop

    """
    def __init__ (self, exception = ValueError("Too many iterations")):
        self.tries_left = 1000
...
class SmartPtrIterator(Iterator):
    ...
    def __next__(self):
        ...
        return ('*0x%x' % val, val.dereference())
...
class RbtreeIterator(Iterator):
    ...
    def __next__(self):
        ...
        result = self.node
        self.count = self.count + 1
        auto_stop = AutoStop(exception=ValueError('RB Tree is too deep'))
        if self.count < self.size:
...
class StdRbtreeIteratorPrinter:
    ...
    def __init__ (self, typename, val):
        ...
        try:
            ...
            self.link_type = nodetype.strip_typedefs()
...
class StdHashtableIterator(Iterator):
    def __init__(self, hashtable):
        try:
            self.node = hashtable['_M_bbegin']['_M_node']['_M_nxt']
        except:
        ...
        try:
            ...
            try:
                self.node_type = find_type(hashtable.type, '__node_type').pointer()
            except:
                ...
                self.node_type = hashtable['_M_begin'].type.target()
