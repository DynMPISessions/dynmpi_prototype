...
class UniquePointerPrinter:
    ...
    def _getV(self):
        ...
        # if impl_type is none, then this is most likely an old compiler
        if impl_type is None or is_specialization_of(impl_type, 'tuple'):
...
class StdListPrinter:
    ...
    class _iterator(Iterator):
        def __init__(self, nodetype, head, backpointer):
    ...
    def __init__(self, typename, val):
        self.typename = strip_versioned_namespace(typename)
        self.val = val
        # ALL6090: Extract the address of the head from the back pointer to head rather
        # than using head.address. This is because head.address is 0 when printing from a GDB
        # convenience variable, which causes the list traversal to never terminate.
        self.backpointer = self.val['_M_impl']['_M_node']['_M_next']['_M_prev']

    def children(self):
        nodetype = find_type(self.val.type, '_Node')
        nodetype = nodetype.strip_typedefs().pointer()
        return self._iterator(nodetype, self.val['_M_impl']['_M_node'], self.backpointer)

    def to_string(self):
        if self.backpointer == self.val['_M_impl']['_M_node']['_M_next']:
    ...
    def to_string(self):
        ...
        if self.new_string:
            ...
            try:
                ...
                # The code above to calculate the length of the string including
                # nulls doesn't work with PGI because it puts the compressed
                # typename _ZSs into the dwarf instead of the full typename that
                # GDB recognises as std::string. So fallback to calling string()
                # which stops at the first null character which is good enough.
                return ptr.string()
...
class Tr1HashtableIterator(Iterator):
    def __init__ (self, hash):
        ...
        try:
            ...
            # The above doesn't work for PGI on Power. Fortunately it doesn't appear to be
            # critical.
...
class StdHashtableIterator(Iterator):
    def __init__(self, hash):
        ...
        try:
            ...
            # PGI on Power doesn't provide debug info for type aliases. So we now look at
            # the return type of a function that we know is of __node_type.
            self.node_type = hash['_M_begin'].type.target()
    ...
    def __next__(self):
        ...
        try:
            ...
            # GDB can't extract the template arguments from PGI types on Power. It's most
            # likely related to GDB not knowing how to demangle PGI's C++ symbols on
            # Power.
            pass
...
def build_libstdcxx_dictionary ():
    ...
    libstdcxx_printer.add('_ZSs', StdStringPrinter) # PGI uses compressed form of std::string in the dwarf
