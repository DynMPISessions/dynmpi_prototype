...
class AutoStop:
    """Helper class for iterators - only do a fixed number of 'operations' per
       call to next. Raises StopIteration if the limit is exceeded - typically
       because we're looking at uninitialized data and are in a loop"""
    def __init__ (self):
        self.tries_left = 1000 # maximum number of ticks allowed per value returned
    
    def tick(self):
        self.tries_left -= 1
        if not self.tries_left:
            raise StopIteration # just pretend to gdb that we've run out of values
...
class SharedPointerPrinter:
    ...
    def children(self):
         v = self.val['_M_ptr']
         return iter([('*0x%x' % v, v.dereference())])
...
class StdRbtreeIteratorPrinter:
    ...
    def to_string (self):
        try:
            ...
            try:                        # Then normal style
                nodetype = gdb.lookup_type('std::_Rb_tree_node< %s >' % valuetype).pointer()
            except RuntimeError:        # Ok, try the intel style
                try:
                    nodetype = gdb.lookup_type('std::_Rb_tree_node<std::set<%s, std::less<%s >, std::allocator<%s > >::value_type >' % (valuetype, valuetype, valuetype)).pointer()
                except RuntimeError:
                    nodetype = gdb.lookup_type('_Rb_tree_node<std::set<%s, std::less<%s >, std::allocator<%s > >::value_type >' % (valuetype, valuetype, valuetype)).pointer()
...
class StdMapPrinter:
    ...
    def children (self):
        try:
            ...
            nodetype = find_type(rep_type, '_Link_type')
            nodetype = node.strip_typedefs()
        except:
            keytype = self.val.type.template_argument(0).const()
            valuetype = self.val.type.template_argument(1)
            mutable_keytype = keytype.unqualified()
            const_keytype = keytype.const()
            types = [ 'std::_Rb_tree_node< std::pair< %s, %s > >' % (keytype, valuetype),
                    # Intel variants
                    'std::_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (keytype, valuetype, keytype, keytype, valuetype),
                    'std::_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (keytype, valuetype, keytype, const_keytype, valuetype),
                    'std::_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (mutable_keytype, valuetype, mutable_keytype, keytype, valuetype),
                    'std::_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (mutable_keytype, valuetype, mutable_keytype, const_keytype, valuetype),
                    '_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (keytype, valuetype, keytype, keytype, valuetype),
                    '_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (keytype, valuetype, keytype, const_keytype, valuetype),
                    ':_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (mutable_keytype, valuetype, mutable_keytype, keytype, valuetype),
                    ':_Rb_tree_node<std::map<%s, %s, std::less<%s >, std::allocator<std::pair<%s, %s > > >::value_type >' % (mutable_keytype, valuetype, mutable_keytype, const_keytype, valuetype) ]
            nodetype = None
            for type in types:
                try:
                    nodetype = gdb.lookup_type(type).pointer()
                    break
                except RuntimeError:
                    pass
            if not nodetype:
...
class StdSetPrinter:
    ...
    def children (self):
        try:
            ...
            # Intel compilers use a different (more verbose) type for the internals of
            # a stl set. If we don't find the correct type at first, look for it using
            # the intel format.  
            nodetype = None
            try:                        # Then normal style
                ...
                try:
                    nodetype = gdb.lookup_type('std::_Rb_tree_node<std::set<%s, std::less<%s >, std::allocator<%s > >::value_type >' % (keytype, keytype, keytype)).pointer()
                except RuntimeError:
                    nodetype = gdb.lookup_type('_Rb_tree_node<std::set<%s, std::less<%s >, std::allocator<%s > >::value_type >' % (keytype, keytype, keytype)).pointer()
...
class StdStringPrinter:
    ...
    def to_string(self):
        ...
        try:
            ...
            i = 0;
            while self.val['_M_dataplus']['_M_p'][i] != 0 and i < 10000:
                    i = i + 1
            len = i
...
class Tr1UnorderedSetPrinter:
    ...
    def hashtable (self):
        value = get_std_printer_value (self) 
        if self.typename.startswith('std::tr1') or value is None:
            return self.val
        return value
...
def get_std_printer_value (printer):
    """ Extract the value of a printer if the printer contains an
    std:: type. If not return None.

    printer -- the printer to operate on
    Return  -- The value to use if the printer holds an std:: type, or None
               if it smells more like a TR1 object.
    """
    stdKey = '_M_h'
    try:
        value = printer.val[stdKey]
    except:
        value = None
