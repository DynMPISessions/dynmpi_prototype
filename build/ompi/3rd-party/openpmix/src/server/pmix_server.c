...
// global variables
PMIX_EXPORT extern pmix_server_globals_t pmix_server_globals = {{{0}}};
                         ...
                         pmix_release_cbfunc_t release_fn, void *release_cbdata)
{
    ...
    for(n=0; n < ninfo; n++){
        if(PMIX_CHECK_KEY(&info[n], PMIX_PSETOP_PRESULT)){
            PMIX_VALUE_UNLOAD(rc, &info[n].value, &pset_name, &sz);
    ...
    for(n = 0; n < pset_size; n++){
        pmix_proc_t *proc = (pmix_proc_t *) &procs_darray->array[n];
    }
