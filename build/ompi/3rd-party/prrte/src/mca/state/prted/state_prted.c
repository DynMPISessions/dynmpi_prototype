...
static void track_procs(int fd, short argc, void *cbdata)
{
    ...
    //printf("%s state:prted:track_procs called for proc %s state %s\n ",PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), PRTE_NAME_PRINT(proc), prte_proc_state_to_str(state));
    ...
    if (NULL == pdata) {
        //printf("%s ERR_NOT_FOUND: state:prted:track_procs called for proc %s state %s\n",
        //             PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), PRTE_NAME_PRINT(proc),
        //             prte_proc_state_to_str(state));
        PRTE_ERROR_LOG(PRTE_ERR_NOT_FOUND);
        goto cleanup;
    ...
    if (PRTE_PROC_STATE_RUNNING == state) {
        ...
        /* track job status */
        /* EDIT: SEND THIS FOR EVERY TERMINATING PROC SO MASTER IS INFORMED */
        ...
        /* EDIT: now do the rest only at local completion as before */
